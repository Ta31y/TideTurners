...
// ===== per-level info (we keep 4, but we spawn the same way every time) =====
const BOARD_DATA = [
  {
  ...
  },
  {
    ...
    lifespan: [
      ...
      "Autoband — heel lang (100+ jaar)",
    ],
    animals: ["Diepzeevissen", "Zeekomkommer"],
    animalsInfo: "Weinig licht; afval blijft lang liggen.",
    types: ["bottle", "straw", "bag", "rings"],
  },
  {
    depthName: "Abyssale vlaktes en troggen (4000–11.000 m)",
    wasteInfo:
      "Plastic + microplastics, oude metalen objecten, militair/industrieel afval.",
    lifespan: ["Wegwerpluier — ~500 jaar", "Tandenborstel — ~500 jaar"],
    animals: ["Diepzeevissen", "Spons", "Kreeftachtigen"],
    animalsInfo: "Extreem diep; afval en microplastics stapelen zich op.",
    types: ["bottle", "straw", "bag", "rings"],
  },
];
...
AFRAME.registerComponent("net-collector", {
  ...
  tick: function () {
    const netPos = this.el.object3D.getWorldPosition(this.tmp1);
...
AFRAME.registerComponent("net-boost", {
  ...
  init: function () {
    const nc = this.el.components["net-collector"];
    if (!nc) return;
    const setBase = () => (nc.data.radius = this.data.base);
    const setBoost = () => (nc.data.radius = this.data.boosted);
...
AFRAME.registerComponent("simple-trash-spawner", {
  ...
  init: function () {
    ...
    for (let i = 0; i < this.data.count; i++) {
      ...
      // POSITION: very close to player
      const x = Math.random() * 6 - 3; // -3 .. 3
      const y = 0.8 + Math.random() * 1.6; // 0.8 .. 2.4
      const z = -2.5 - Math.random() * 6; // -2.5 .. -8.5
      e.setAttribute("position", `${x} ${y} ${z}`);

      // float
      const toX = x + (Math.random() * 0.5 - 0.25);
      const toY = y + (0.3 + Math.random() * 0.6);
      ...
      e.setAttribute(
        ...
        }; to: ${(Math.random() * 360) | 0} ${(Math.random() * 360) | 0} ${
          (Math.random() * 360) | 0
        }`
      );
...
AFRAME.registerComponent("game-manager", {
  init: function () {
    ...
    this.infoPanelText = $("#infoPanelText");
    this.infoPanelBtn = $("#infoPanelBtn");
    this.beach = $("#beachEnv");
    ...
    // desktop click anywhere to start
    window.addEventListener("click", () => {
      if (this.i < 0) this.startGame();
    });

    // VR trigger to start
    if (this.rightCtrl) {
      this.rightCtrl.addEventListener("triggerdown", () => {
        if (this.i < 0) this.startGame();
        else if (this.infoPanel.getAttribute("visible")) this.closeInfo();
      });
    }

    // click directly on the 3D start panel
    this.startPanel.addEventListener("click", () => this.startGame());
  },

  startGame: function () {
    if (this.i >= 0) return;
    this.startPanel.setAttribute("visible", false);
    // push rig a bit forward into water
    this.rig.object3D.position.set(0, 1.6, -1);
    this.startLevel(0);
  },
  ...
  startLevel: function (idx) {
    ...
    this.sky.setAttribute("color", skyColors[idx]);
  ...
  endLevel: function () {
    ...
    const extra = this.buildInfo();

    this.infoPanelText.setAttribute(
      "text",
      `value: Einde level ${this.i + 1}: ${
        names[this.i]
      }\nGevangen: ${caught}\nTotaal: ${TOTAL}\n\n${extra}; align: center; color: #163344; width: 2.1; wrapCount: 36`
    );
  ...
  finish: function () {
    this.infoPanelText.setAttribute(
      "text",
      `value: Einde level 4\nTotaal: ${TOTAL}\n\nBedankt voor het spelen!\n(30s); align: center; color: #163344; width: 2.1; wrapCount: 36`
    );
    this.infoPanel.setAttribute("visible", true);
    setTimeout(() => {
      this.infoPanel.setAttribute("visible", false);
  ...
  tick: function (t, dt) {
    ...
    if (TRASH.length === 0) this.endLevel();
  },
});
...
AFRAME.registerComponent("vr-swim", {
  ...
  tick: function (t, dt) {
    ...
    // forward dir from camera
    const forward = new THREE.Vector3(0, 0, -1);
    camObj.getWorldDirection(forward);
    forward.y = 0;
    forward.normalize();

    // swim forward/back (RIGHT stick Y)
    const forwardMove = -this.rightAxis.y * this.data.moveSpeed * dts;
    rigObj.position.addScaledVector(forward, forwardMove);

    // strafe (RIGHT stick X)
    const right = new THREE.Vector3()
      .crossVectors(forward, new THREE.Vector3(0, 1, 0))
      .normalize();
    const sideMove = this.rightAxis.x * this.data.moveSpeed * dts;
    rigObj.position.addScaledVector(right, sideMove);
...
AFRAME.registerComponent("hand-vr-sync", {
  init: function () {
    ...
    this.rig = this.el;
    this.leftRig = $("#leftHand");
    this.rightRig = $("#rightHand");
    this.net = $("#net");
  ...
  toVR: function () {
    if (this.leftCtrl && this.leftRig) {
      this.leftCtrl.appendChild(this.leftRig);
      this.leftRig.setAttribute("visible", true);
      this.leftRig.object3D.position.set(0.02, -0.03, -0.13);
    }
    if (this.rightCtrl && this.rightRig) {
      this.rightCtrl.appendChild(this.rightRig);
      this.rightRig.setAttribute("visible", true);
      this.rightRig.object3D.position.set(0.02, -0.03, -0.13);
    }
    if (this.net && this.rightRig) {
      this.rightRig.appendChild(this.net);
      this.net.object3D.position.set(0, -0.02, -0.18);
    }
  },
  toDesktop: function () {
    if (this.rig && this.leftRig) {
      this.rig.appendChild(this.leftRig);
      this.leftRig.setAttribute("visible", true);
      this.leftRig.object3D.position.set(-0.25, -0.15, -0.5);
    }
    if (this.rig && this.rightRig) {
      this.rig.appendChild(this.rightRig);
      this.rightRig.setAttribute("visible", true);
      this.rightRig.object3D.position.set(0.25, -0.15, -0.5);
    }
  },
});
