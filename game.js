...
AFRAME.registerComponent("net-collector", {
  ...
  tick: function () {
    const pos = this.el.object3D.getWorldPosition(this.tmp1);
    for (let i = TRASH.length - 1; i >= 0; i--) {
      ...
      if (p.distanceTo(pos) < this.data.radius) {
        // send event
...
AFRAME.registerComponent("net-boost", {
  ...
  init: function () {
    const comp = this.el.components["net-collector"];
    if (!comp) return;
    const setBase = () => (comp.data.radius = this.data.base);
    const setBoost = () => (comp.data.radius = this.data.boosted);
...
AFRAME.registerComponent("simple-trash-spawner", {
  ...
  init: function () {
    ...
    for (let i = 0; i < this.data.count; i++) {
      ...
      const y = 0.5 + Math.random() * 1.8; // 0.5 .. 2.3
      const z = -3 - Math.random() * 10; // -3 .. -13
      e.setAttribute("position", `${x} ${y} ${z}`);

      const toX = x + (Math.random() * 0.5 - 0.25);
      const toY = y + (0.4 + Math.random() * 0.9);
...
AFRAME.registerComponent("game-manager", {
  ...
  startGame: function () {
    ...
    // move player a little into water
  ...
  startLevel: function (idx) {
    ...
    // spawn trash (same for every level, just darker)
  ...
  buildInfoText: function () {
    ...
    let col = keys.length
      ? keys
          .map((k) => {
            const n = this.stats[k];
            const meta = TYPE_META[k] || { label: k, icon: "•" };
            return `${meta.icon} ${meta.label}: ${n}`;
          })
          .join("\n")
      : "-";
  ...
  endLevel: function () {
    ...
    const body = this.buildInfoText();
    this.infoPanelText.setAttribute(
      "text",
      `value: Einde level ${this.i + 1}: ${
        names[this.i]
      }\nGevangen: ${caught}\nTotaal: ${TOTAL}\n\n${body}; align: center; color: #163344; width: 2.1; wrapCount: 36`
    );
    this.infoPanel.setAttribute("visible", true);

    this.infoPanelBtn.onclick = null;
    this.infoPanelBtn.addEventListener(
      "click",
      () => {
  ...
  finish: function () {
    this.infoPanelText.setAttribute(
      "text",
      `value: Einde level 4\nTotaal: ${TOTAL}\n\nBedankt voor het spelen!\nSluit in 30 seconden...; align: center; color: #163344; width: 2.1; wrapCount: 36`
    );
    this.infoPanel.setAttribute("visible", true);
