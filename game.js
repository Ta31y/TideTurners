...
const BOARD_DATA = [
  ...
  },
  {
    ...
    wasteInfo: "Metaal, glas, plastic zakken, verroeste visnetten.",
...
// ---------- simple drift (we use movement-controls now, but we keep this for auto swim) ----------
...
AFRAME.registerComponent("net-collector", {
  ...
  sync: function () {
    if (this.data.score3D) {
      this.data.score3D.setAttribute(
        "text",
        "value",
        "Trash: " + this.score
      );
...
// ---------- boost on mouse (desktop) ----------
AFRAME.registerComponent("net-boost", {
  schema: { base: { default: 0.5 }, boosted: { default: 0.8 } },
  init: function () {
...
AFRAME.registerComponent("simple-trash-spawner", {
  ...
  init: function () {
    ...
    for (let i = 0; i < this.data.count; i++) {
      ...
      // 👇 CLOSER to player: z between -3 and -25
      ...
      const toY = y + (0.6 + Math.random() * 1.0);
...
AFRAME.registerComponent("game-manager", {
  init: function () {
    ...
    // 3D start
    this.vrStart = $("#vrStart");
    this.vrStartBtn = $("#vrStartBtn");
    ...
    // VR start
    if (this.vrStartBtn) {
      this.vrStartBtn.addEventListener("click", () => this.startTransition());
    }

    // also start on Enter
    window.addEventListener("keydown", (e) => {
      if (e.code === "Enter" && this.i < 0) this.startTransition();
    });
  },
  ...
  startTransition: function () {
    ...
    // hide VR start
    if (this.vrStart) this.vrStart.setAttribute("visible", false);
  ...
  showThankYouScene: function (total) {
    ...
    const step = (ts) => {
      if (!t0) t0 = ts;
      const k = Math.min(1, (ts - t0) / 3000);
      obj.position.y = THREE.MathUtils.lerp(startY, targetY, k);
      if (k < 1) requestAnimationFrame(step);
      else this.turtleThanks(total);
    };
    requestAnimationFrame(step);
  },

  turtleThanks: function (total) {
    const dlg = $("#dialog");
    const btn = $("#dialogBtn");
    dlg.querySelector(
      "#dialogText"
    ).innerHTML = `🐢 Thanks! You collected <strong>${total}</strong> pieces.<br>Play again?`;
